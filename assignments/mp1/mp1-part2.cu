...
__global__ void force_eval(float4 *set_A, float4 *set_B, int * indices, float4 *force_vectors, int array_length)
{
  ...
  if (indices[i] < array_length && indices[i] >= 0)
  {
    // implement forcecalc here
    float4 A = set_A[i], B = set_B[indices[i]];
    float x = B.x - A.x;
    float y = B.y - A.y;
    float z = B.z - A.z;
    float rsq = x*x + y*y + z*z;
...
{ 
  ...
  float4 *d_set_A, *d_set_B, *d_force_vectors;
  int *d_indices;
  ...
  if(d_set_A == 0 || d_set_B == 0 || d_force_vectors == 0 || d_indices == 0)
  {
  ...
  start_timer(&timer);
  // launch kernel
  force_eval <<<(num_elements+1023)/1024, 1024>>> (d_set_A, d_set_B, d_indices, d_force_vectors, num_elements);
