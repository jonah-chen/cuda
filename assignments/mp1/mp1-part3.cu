...
__global__
void graph_propagate(unsigned int *graph_indices, unsigned int *graph_edges, float *graph_nodes_in, float *graph_nodes_out, float * inv_edges_per_node, int array_length)
{
                          ...
                          int avg_edges)
{
  ...
  // allocate memory
  cudaMalloc((void**)&d_graph_indices, (num_elements+1)*sizeof(int));
  cudaMalloc((void**)&d_graph_edges, num_elements*avg_edges*sizeof(int));
  ...
  cudaMemcpy(d_graph_indices, h_graph_indices, (num_elements+1)*sizeof(int), cudaMemcpyHostToDevice);
  cudaMemcpy(d_graph_edges, h_graph_edges, num_elements*avg_edges*sizeof(int), cudaMemcpyHostToDevice);
  ...
  for(int iter = 0; iter < nr_iterations; iter+=2)
  {
    graph_propagate <<<(1023*num_elements)/1024, 1024>>> (d_graph_indices, d_graph_edges, d_graph_nodes_input, d_graph_nodes_result, d_inv_edges_per_node, num_elements);
    graph_propagate <<<(1023*num_elements)/1024, 1024>>> (d_graph_indices, d_graph_edges, d_graph_nodes_result, d_graph_nodes_input, d_inv_edges_per_node, num_elements);
  }
